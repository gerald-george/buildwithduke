...
export const onRequestGet: PagesFunction<Env> = async ({ env }) => {
  ...
  try {
    const [projects, pricing, testimonials, blogPosts, settings, pages] = await Promise.all([
      ...
      rows(env.DB, "SELECT id, slug, title, excerpt, body, published_at, seo_title, meta_description, cover_image, focus_keyword, source_urls, ai_generated FROM blog_posts WHERE status = 'published' AND (published_at IS NULL OR published_at <= datetime('now')) ORDER BY published_at DESC"),
      rows(env.DB, "SELECT key, value FROM business_settings WHERE key IN ('business_name','contact_email','phone_number','phone_display','whatsapp_number','service_area','response_time','github_url','instagram_url','linkedin_url','accepting_projects','visitor_guide_enabled') ORDER BY key ASC"),
