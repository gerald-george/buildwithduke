...
export const onRequestPost: PagesFunction<AdminEnv> = async ({ request, env }) => {
  ...
  let email = env.ADMIN_EMAIL?.toLowerCase();
  ...
  if ((!email || !storedHash) && env.DB) {
    ...
    email = user?.email.toLowerCase(); storedHash = user?.password_hash;
  }
  const suppliedHash = await passwordDigest(body.password);
  const expectedHash = storedHash?.replace(/^sha256:/, "").toLowerCase();
  if (!email || email !== body.email.toLowerCase() || !expectedHash || suppliedHash !== expectedHash) {
