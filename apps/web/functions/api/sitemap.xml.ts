...
const staticRoutes = ["/", "/projects", "/services", "/pricing", "/about", "/contact", "/cv", "/articles", "/privacy", "/cookies", "/terms"];
...
export const onRequestGet: PagesFunction<Env> = async ({ env }) => {
  ...
  const dynamic = env.DB ? await Promise.all([
    ...
    env.DB.prepare("SELECT slug, COALESCE(published_at, created_at) AS updated_at FROM blog_posts WHERE status = 'published' AND (published_at IS NULL OR datetime(published_at) <= datetime('now')) ORDER BY datetime(published_at) DESC, created_at DESC").all().catch(() => ({ results: [] })),
  ]) : [{ results: [] }, { results: [] }];
  ...
  for (const row of dynamic[1].results as Array<Record<string, unknown>>) entries.set(`/articles/${row.slug}`, dateOnly(row.updated_at));
  const urls = [...entries].map(([route, updated]) => `<url><loc>${base}${route}</loc>${updated ? `<lastmod>${updated}</lastmod>` : ""}${route === "/" ? "<priority>1.0</priority>" : route === "/projects" || route === "/contact" || route === "/articles" ? "<priority>0.9</priority>" : ""}</url>`).join("");
