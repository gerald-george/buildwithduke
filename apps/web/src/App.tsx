  ...
  return <>
    <section className="hero">
      ...
      <div className="shell hero-inner">
        ...
        <div className="hero-system" aria-label="How Duke works">
          <div className="system-head"><span>How I work</span><CircleDot size={16} /></div>
          <div className="system-console">
            ...
            <div className="system-ready"><i /> ready for a useful problem</div>
          </div>
    ...
    <section className="section shell" id="work"><SectionHead label="01 / Selected work" title={pageText(copy, "work_heading")} copy={pageText(copy, "work_intro")} />
      <div className="featured-grid">{projects.filter(p => p.featured).map(project => <TerminalWindow key={project.slug} label="Project preview" className="project-card">
    ...
    <section className="proof-band"><div className="shell proof-grid"><div className="whoami"><span className="prompt">A little about me</span><blockquote>“{pageText(copy, "proof_quote")}”</blockquote></div><div className="proof-list">
    ...
    <section className="section shell"><SectionHead label="02 / The process" title={pageText(copy, "process_heading")} copy={pageText(copy, "process_intro")} /><div className="process-grid">{[
      ['01', 'Discover', 'Map the problem, users, constraints and the outcome worth paying for.'],
      ['02', 'Design', 'Shape the flow and visual system before expensive build decisions settle.'],
      ['03', 'Build', 'Ship in visible increments with clear interfaces and dependable connections.'],
      ['04', 'Launch', 'Test the real journey, document what matters and stay close after go-live.'],
    ].map(([n, title, copy]) => <div key={n} className="process-step"><span>{n}</span><h3>{title}</h3><p>{copy}</p></div>)}</div></section>
...
function Testimonials({ items }: { items: Testimonial[] }) {
  ...
  return <section className="section shell testimonial-section" aria-label="Client feedback"><SectionHead label="Client feedback" title={pageText(copy, "testimonials_heading")} /><TerminalWindow label="From a client" className="testimonial-terminal"><blockquote>“<ScrollTypingText key={`${item.id}-${active}`} text={item.quote} />”</blockquote><div className="testimonial-meta"><span><strong>{item.authorName}</strong>{[item.authorRole, item.company].filter(Boolean).join(" · ")}</span>{items.length > 1 && <div><button aria-label="Previous testimonial" onClick={() => setActive((active - 1 + items.length) % items.length)}>←</button><span>{active + 1} / {items.length}</span><button aria-label="Next testimonial" onClick={() => setActive((active + 1) % items.length)}>→</button></div>}</div></TerminalWindow></section>;
}
...
function Projects() {
  ...
  return <><PageHero label="Selected work" title={pageText(copy, "hero_title")} copy={pageText(copy, "hero_intro")} />
