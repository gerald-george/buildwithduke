...
function About() { const { settings, page } = useContent(); const copy = page("about"); const titles = pageList(copy, "credential_titles"); const details = pageList(copy, "credential_details"); const icons = [<GraduationCap />, <Award />, <Globe2 />, <BookOpen />]; return <><PageHero label="/about" title={pageText(copy, "hero_title")} copy={pageText(copy, "hero_intro")} /><section className="section shell about-intro"><div className="portrait-frame"><div className="terminal-bar"><span className="window-dots"><i /><i /><i /></span><span>duke.profile</span></div><div className="portrait-media"><img src="/headshot.png" alt="Portrait of Duke Chijimaka Jonathan" width="1254" height="1254" /><img className="portrait-glitch" src="/headshot.png" alt="" width="1254" height="1254" aria-hidden="true" /><span className="portrait-static" aria-hidden="true" /></div><span className="portrait-status"><i /> {settings.accepting_projects === "true" ? "open to useful problems" : "currently fully booked"}</span></div><div className="about-copy"><span className="kicker">The short version</span><RevealHeading text={pageText(copy, "profile_heading")} variant="decrypt" /><p>{pageText(copy, "profile_paragraph_1")}</p><p>{pageText(copy, "profile_paragraph_2")}</p><p>{pageText(copy, "profile_paragraph_3")}</p><div className="button-row"><Link className="button button-primary" to="/cv">Read the formal version <ArrowRight size={17} /></Link><a className="button button-ghost" href={settings.github_url} target="_blank" rel="noreferrer"><Github size={17} /> GitHub</a></div></div></section>
...
function ManagedCV() {
  ...
  return <><PageHero label="/cv" title={pageText(copy, "hero_title")} copy={pageText(copy, "hero_intro")} /><section className="section shell cv-layout"><aside><div className="portrait-mini"><img src="/headshot.png" alt="Portrait of Duke Chijimaka Jonathan" width="1254" height="1254" /></div><h2>Duke Chijimaka Jonathan</h2><p>{pageList(copy, "sidebar_roles").map(role => <span className="cv-role" key={role}>{role}</span>)}</p><a href={`mailto:${CONTACT_EMAIL}`}>{CONTACT_EMAIL}</a><span>{pageText(copy, "sidebar_location")}</span><a className="button button-primary" href="/duke-chijimaka-jonathan-cv.pdf" download><Download size={17} /> Download CV</a></aside><div className="cv-main"><CVSection title="Profile"><p>{pageText(copy, "profile_copy")}</p></CVSection><CVSection title="Selected outcomes"><ul>{pageList(copy, "outcome_items").map(item => <li key={item}>{item}</li>)}</ul></CVSection><CVSection title="Experience">{experience.map(([title, detail, metaText]) => <div className="cv-entry cv-entry-detail" key={title}><div><strong>{title}</strong><p>{detail}</p></div><span>{metaText}</span></div>)}</CVSection><CVSection title="Selected technical work">{technical.map(([title, detail, metaText]) => <div className="cv-entry cv-entry-detail" key={title}><div><strong>{title}</strong><p>{detail}</p></div><span>{metaText}</span></div>)}</CVSection><CVSection title="Core capabilities"><div className="tag-row">{pageList(copy, "capabilities").map(item => <span key={item}>{item}</span>)}</div></CVSection><CVSection title="Education & recognition">{education.map(([title, metaText]) => <div className="cv-entry" key={title}><strong>{title}</strong><span>{metaText}</span></div>)}</CVSection><CVSection title="Certifications">{certifications.map(([title, metaText]) => <div className="cv-entry" key={title}><strong>{title}</strong><span>{metaText}</span></div>)}</CVSection></div></section></>;
}
...
function Blog() { const { blogPosts, page } = useContent(); const copy = page("blog"); return <><PageHero label="/build-log" title={pageText(copy, "hero_title")} copy={pageText(copy, "hero_intro")} /><section className="section shell">{blogPosts.length ? <div className="blog-grid">{blogPosts.map((post, i) => <article key={post.id}><span className="kicker">Build log · {String(i + 1).padStart(2, "0")}</span><RevealHeading text={post.title} variant="decrypt" /><p>{post.excerpt}</p>{post.publishedAt && <time className="muted" dateTime={post.publishedAt}>{new Date(post.publishedAt).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}</time>}<Link className="blog-read-link" to={`/blog/${post.slug}`}>Read article <ArrowRight size={15} /></Link></article>)}</div> : <TerminalWindow label="build-log/status"><div className="empty-state"><BookOpen /><RevealHeading text={pageText(copy, "empty_heading")} variant="type" /><p>{pageText(copy, "empty_copy")}</p></div></TerminalWindow>}</section></> }
...
function BlogDetail() {
  ...
  if (!post) return <section className="not-found"><span>404</span><h1>That article is not published.</h1><p>It may still be in draft, or the address may have changed.</p><Link className="button button-primary" to="/blog">Return to build log <ArrowRight size={16} /></Link></section>;
  const words = (post.body || "").replace(/<[^>]*>/g, " ").trim().split(/\s+/).filter(Boolean).length;
  return <><section className="blog-article-hero"><div className="shell"><Link className="back-link" to="/blog">← Back to build log</Link><span className="kicker">Build log</span><h1>{post.title}</h1><p>{post.excerpt}</p><div><time dateTime={post.publishedAt}>{post.publishedAt ? new Date(post.publishedAt).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" }) : ""}</time><span>{Math.max(1, Math.ceil(words / 220))} min read</span></div></div></section><article className="section shell blog-article"><div className="blog-prose" dangerouslySetInnerHTML={{ __html: cleanBody }} /></article><CTA /></>;
}

const legal = {
  privacy: { title: "Privacy policy", updated: "19 July 2026", intro: "This policy explains what information I collect through buildwithduke, why I collect it, and the choices you have.", sections: [["Who controls your data", "Duke Chijimaka Jonathan, trading as buildwithduke, is the data controller. Contact buildwithduke@outlook.com for any privacy request."], ["What I collect", "When you send an enquiry, I collect your name, email, optional company, project details, consent record and the business correspondence needed to manage that enquiry. Basic server security logs may also record an IP address temporarily."], ["Why and how long", "I use enquiry data only to respond, prepare a proposal and keep necessary business records. Unconverted enquiries are reviewed and deleted after 12 months."], ["Your rights", "Under UK data protection law you may ask to access, correct, erase, restrict or export your personal information, and you may object to processing."], ["Service providers", "Cloudflare processes site delivery, security and stored lead data. Google Apps Script and Gmail process private contact-form notifications sent to the notification mailbox; business follow-up is handled separately through Outlook. SerpApi and OpenRouter process public research prompts for article automation, without receiving lead details. Data is not sold or used for unrelated marketing."]]},
...
function Legal({ type }: { type: keyof typeof legal }) { const copy = useContent().page(type); const titles = pageList(copy, "section_titles"); const copies = pageList(copy, "section_copies"); return <><PageHero label="/legal" title={pageText(copy, "hero_title")} copy={pageText(copy, "hero_intro")}><span className="legal-date">Last updated: {pageText(copy, "updated_date")}</span></PageHero><section className="section shell legal-copy">{titles.map((title, index) => <section key={title}><span>0{index + 1}</span><div><RevealHeading text={title} variant="type" /><p>{copies[index]}</p></div></section>)}</section></> }

function CTA() { const copy = useContent().page("common"); return <section className="final-cta"><div className="shell"><span className="kicker">$ sudo start-project</span><RevealHeading text={pageText(copy, "cta_heading")} variant="decrypt" /><p>{pageText(copy, "cta_copy")}</p><div className="button-row"><Link className="button button-primary" to="/contact">Start a project <ArrowRight size={17} /></Link><WhatsAppLink /></div></div></section> }
