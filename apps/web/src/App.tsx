...
import { ArrowDown, ArrowRight, ArrowUpRight, Award, BookOpen, Bot, Check, CheckCircle2, CircleDot, Clock3, Code2, Download, ExternalLink, Github, GraduationCap, Mail, MapPin, MessageCircle, Send, ShieldCheck, Sparkles, Workflow, Wrench } from "lucide-react";
...
function Home() {
  ...
  return <>
    ...
    <section className="proof-band"><div className="shell proof-grid"><div className="whoami"><span className="prompt">$ whoami</span><blockquote>“A systems thinker who enjoys the moment a messy thing finally makes sense.”</blockquote><span className="cursor-line">duke@buildwithduke <i /></span><small>Outcome figures are Duke’s reported project results.</small></div><div className="proof-list">
...
function ProjectDetail() {
  ...
  return <><section className="case-hero"><div className="shell"><Link className="back-link" to="/projects">← All projects</Link><div className="case-title"><div><span className="kicker">{project.eyebrow}</span><RevealHeading as="h1" text={project.title} variant="type" /><p>{project.description}</p><div className="button-row"><a className="button button-primary" href={project.liveUrl} target="_blank" rel="noreferrer">Visit project <ExternalLink size={17} /></a><Link className="button button-ghost" to="/contact">Build something useful</Link></div></div><TerminalWindow label={`${project.slug}.live`}><ProjectVisual image={project.image} title={project.title} /></TerminalWindow></div></div></section>
    <section className="section shell case-body"><aside><span>Project index</span><a href="#problem">01 Problem</a><a href="#build">02 Build</a><a href="#result">03 Result</a></aside><div className="case-copy"><section id="problem"><span className="kicker">01 / The problem</span><RevealHeading text="What needed to change." variant="decrypt" /><p>{project.problem}</p></section><section id="build"><span className="kicker">02 / The build</span><RevealHeading text="The useful part." variant="decrypt" /><p>{project.solution}</p><div className="stack-list">{project.stack.map(s => <span key={s}><Check size={14} />{s}</span>)}</div></section><section id="result"><span className="kicker">03 / The result</span><RevealHeading text="What shipped." variant="decrypt" /><p>{project.result}</p>{project.demo && <div className="callout"><ShieldCheck size={20} /><span><strong>Honest demo label</strong>This is a demo build with a live client agreement in place. It is not presented as the client’s production storefront.</span></div>}</section></div></section>
...
function Pricing() { const { pricing } = useContent(); return <><PageHero label="/pricing" title="Straightforward pricing, no surprises." copy="Five practical starting points in GBP. Final scope and payment milestones are agreed in writing before work begins." /><section className="section shell"><div className="pricing-full">{pricing.map(t => <PriceCard key={t.name} tier={t} />)}</div><div className="payment-note"><ShieldCheck /><div><strong>Clear commercial basics.</strong><p>Bank transfer is the only payment method currently available. No VAT is currently charged. You retain ownership after final payment.</p></div></div></section><section className="section section-lined"><div className="shell faq-section"><SectionHead label="Pricing FAQ" title="The practical bits." /><FAQ items={faq} /></div></section><CTA /></> }

function About() { return <><PageHero label="/about" title="Hi, I’m Duke." copy="I like systems, clean hand-offs, and the exact moment a complicated thing becomes understandable." /><section className="section shell about-intro"><div className="portrait-frame"><div className="terminal-bar"><span className="window-dots"><i /><i /><i /></span><span>duke.profile</span></div><img src="/headshot.png" alt="Portrait of Duke Chijimaka Jonathan" width="1254" height="1254" /><span className="portrait-status"><i /> open to useful problems</span></div><div className="about-copy"><span className="kicker">The short version</span><RevealHeading text="Library-science rigour. Product-builder energy." variant="decrypt" /><p>I’m a full-stack developer and AI automation specialist who started with a First-Class degree in Library and Information Science. That background still shows up in how I work: understand the information, understand the people, then make the system make sense.</p><p>I use agentic, AI-assisted development to move faster, but I care more about judgement than novelty. The output still has to be legible, maintainable and useful when the demo is over.</p><p>Away from a screen, I tend to drift toward origami, astronomy and theology: three different ways of asking what structure is hiding underneath the surface.</p><div className="button-row"><Link className="button button-primary" to="/cv">Read the formal version <ArrowRight size={17} /></Link><a className="button button-ghost" href={GITHUB_URL} target="_blank" rel="noreferrer"><Github size={17} /> GitHub</a></div></div></section>
