...
function PageMeta() {
  ...
  const { projects, blogPosts } = useContent();
  useEffect(() => {
    ...
    const value = project ? [`${project.title} case study — buildwithduke`, project.description] : article ? [article.seoTitle || `${article.title} — buildwithduke`, article.metaDescription || article.excerpt] : meta[key] || ["buildwithduke", "Full-stack products and useful automation."];
...
function About() { const { settings } = useContent(); return <><PageHero label="/about" title="Hi, I’m Duke." copy="I like systems, clean hand-offs, and the exact moment a complicated thing becomes understandable." /><section className="section shell about-intro"><div className="portrait-frame"><div className="terminal-bar"><span className="window-dots"><i /><i /><i /></span><span>duke.profile</span></div><div className="portrait-media"><img src="/headshot.png" alt="Portrait of Duke Chijimaka Jonathan" width="1254" height="1254" /><img className="portrait-glitch" src="/headshot.png" alt="" width="1254" height="1254" aria-hidden="true" /><span className="portrait-static" aria-hidden="true" /></div><span className="portrait-status"><i /> {settings.accepting_projects === "true" ? "open to useful problems" : "currently fully booked"}</span></div><div className="about-copy"><span className="kicker">The short version</span><RevealHeading text="Library-science rigour. Product-builder energy." variant="decrypt" /><p>I’m a full-stack developer and AI automation specialist with a First-Class B.LIS (4.6/5.0) from the University of Port Harcourt. That information-science background still shapes the work: understand the information, understand the people, then make the system make sense.</p><p>I build with React, Cloudflare, Python and n8n, using agentic AI-assisted workflows to move quickly without outsourcing judgement. The output still has to be testable, maintainable and useful when the novelty wears off.</p><p>Away from a screen, I tend to drift toward origami, astronomy and theology: three different ways of asking what structure is hiding underneath the surface.</p><div className="button-row"><Link className="button button-primary" to="/cv">Read the formal version <ArrowRight size={17} /></Link><a className="button button-ghost" href={settings.github_url} target="_blank" rel="noreferrer"><Github size={17} /> GitHub</a></div></div></section>
...
function Contact() {
  ...
  const { currency, settings } = useContent();
  ...
  return <><PageHero label="/contact" title="Let’s talk." copy={`Tell me what is stuck, missing or ready to become real. I usually reply ${settings.response_time}.`} /><section className="section shell contact-grid"><div className="contact-aside"><TerminalWindow label="contact.channels"><div className="contact-lines"><a href={`mailto:${settings.contact_email}`}><Mail /><span><small>Email</small>{settings.contact_email}</span></a><a href={`tel:${settings.phone_number}`}><MessageCircle /><span><small>Phone / WhatsApp</small>{settings.phone_display}</span></a><a href={settings.github_url} target="_blank" rel="noreferrer"><Github /><span><small>GitHub</small>View profile</span></a><div><MapPin /><span><small>Service area</small>{settings.service_area}</span></div></div></TerminalWindow><div className="contact-note"><Clock3 /><p><strong>Prefer the fast lane?</strong><span>WhatsApp is often the quickest route for a first conversation.</span></p><WhatsAppLink /></div></div>
