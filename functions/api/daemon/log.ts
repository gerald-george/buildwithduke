...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  await env.DB.prepare("INSERT INTO daemon_events (id, event, path, created_at) VALUES (?, ?, ?, datetime('now'))")
    .bind(crypto.randomUUID(), body.event.slice(0, 80), body.path?.slice(0, 160) || null).run();
